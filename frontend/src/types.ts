...
export interface Configuration {
  ...
  costPerResourcePerMonth: number; // 4L = 400000
...
export interface BlockConfiguration {
  ...
  complexityFactorLevels?: Record<string, string>; // For factors with levels
  dft: boolean;
...
export interface FullChipConfiguration {
  ...
  designMaturity?: 'new_design' | 'revision' | null; // Design maturity for Full Chip
  powerDomains?: 'none' | 'non_nested' | 'nested' | null; // Power domains for Full Chip
  glueLogic?: boolean;
  instanceCount?: number;
  numberOfBlocks?: number; // Number of blocks for Full Chip Only mode
}
...
export interface ProjectConfiguration {
  ...
  isFullChipOnly?: boolean; // New flag for Full Chip Configuration only (no blocks)
  additionalFactors?: Record<string, string>; // DEPRECATED: Not used in calculations
}
